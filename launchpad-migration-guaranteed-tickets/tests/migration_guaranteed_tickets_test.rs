...
mod migration_guaranteed_tickets_setup;

use migration_guaranteed_tickets_setup::{
...
fn confirm_all_test() {
    ...
    for (i, p) in participants.iter().enumerate() {
        ...
        lp_setup.b_mock.check_moax_balance(p, &remaining_balance);
        lp_setup
            .b_mock
            .check_dct_balance(p, LAUNCHPAD_TOKEN_ID, &rust_biguint!(0));
    }
    lp_setup
        .b_mock
        .check_moax_balance(&lp_setup.owner_address, &rust_biguint!(0));
    ...
    for p in participants.iter() {
        ...
        lp_setup.b_mock.check_moax_balance(p, &remaining_balance);
        lp_setup.b_mock.check_dct_balance(
            p,
            LAUNCHPAD_TOKEN_ID,
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&lp_setup.owner_address, &rust_biguint!(TICKET_COST * 3));
}
...
fn add_migration_guaranteed_tickets_distribution_isolated_steps_scenario_test() {
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&participants[2], &rust_biguint!(2 * TICKET_COST));
...
fn add_migration_guaranteed_tickets_distribution_and_claim_scenario_test() {
  ...
  // check MOAX balances of participants before they claim
  let base_user_balance = rust_biguint!(TICKET_COST * MAX_TIER_TICKETS as u64);
  lp_setup.b_mock.check_moax_balance(&participants[0], &base_user_balance);
  lp_setup.b_mock.check_moax_balance(&participants[1], &base_user_balance);
  lp_setup.b_mock.check_moax_balance(&participants[2], &(&base_user_balance - TICKET_COST * 3));
  lp_setup.b_mock.check_moax_balance(&participants[3], &(&base_user_balance - TICKET_COST));
  lp_setup.b_mock.check_moax_balance(&participants[4], &(&base_user_balance * 2_u64 - TICKET_COST * 6));
  ...
  // check MOAX and launchpad token balance for the owner before users claim
  lp_setup
    .b_mock
    .check_moax_balance(&lp_setup.owner_address, &rust_biguint!(0));
  lp_setup
  ...
  lp_setup
      .b_mock
      .check_moax_balance(&participants[3], &rust_biguint!(2 * TICKET_COST));
  ...
  lp_setup
      .b_mock
      .check_moax_balance(&participants[4], &rust_biguint!(3 * TICKET_COST));
  ...
  // Owner claims. All nr_winning_tickets are sold for MOAX. No launchpad tokens refunded.
  lp_setup.claim_owner().assert_ok();

  lp_setup.b_mock.check_moax_balance(
      &lp_setup.owner_address,
...
fn condition_checks_test() {
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&participants[2], &rust_biguint!(TICKET_COST));
...
fn blacklist_scenario_test() {
    ...
    let second_new_participant_moax_balance = TICKET_COST * second_new_participant_tickets;
    let second_new_participant = lp_setup
        .b_mock
        .create_user_account(&rust_biguint!(second_new_participant_moax_balance));
    participants.push(second_new_participant.clone());
    ...
    lp_setup.b_mock.check_moax_balance(
        ...
        &rust_biguint!(second_new_participant_moax_balance),
    );
    lp_setup
        .confirm(
            &second_new_participant,
    ...
    lp_setup.b_mock.check_moax_balance(
        &second_new_participant,
        &rust_biguint!(
            second_new_participant_moax_balance - second_new_participant_tickets * TICKET_COST
        ),
    );
    ...
    lp_setup.b_mock.check_moax_balance(
        &second_new_participant,
        &rust_biguint!(second_new_participant_moax_balance - TICKET_COST),
    );
    lp_setup.b_mock.check_dct_balance(
        &second_new_participant,
        LAUNCHPAD_TOKEN_ID,
...
fn confirm_less_tickets_than_total_available_scenario_test() {
    ...
    lp_setup
        .b_mock
        .check_moax_balance(lp_setup.lp_wrapper.address_ref(), &rust_biguint!(0));
