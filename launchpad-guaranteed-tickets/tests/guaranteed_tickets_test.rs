...
fn add_migration_guaranteed_tickets_distribution_and_claim_scenario_test() {
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&participants[0], &base_user_balance);
    lp_setup
        .b_mock
        .check_moax_balance(&participants[1], &base_user_balance);
    lp_setup
        .b_mock
        .check_moax_balance(&participants[2], &(&base_user_balance - TICKET_COST * 3));
    lp_setup
        .b_mock
        .check_moax_balance(&participants[3], &(&base_user_balance - TICKET_COST));
