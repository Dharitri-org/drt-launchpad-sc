...
use dharitri_sc::{codec::Empty, types::MultiValueEncoded};
...
fn confirm_test() {
    ...
    lp_setup
        .b_mock
        .set_moax_balance(&users[0], &rust_biguint!(1_000_000_000));
    lp_setup
        .confirm_nft(&users[0])
...
fn claim_test() {
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&users[0], &rust_biguint!(0));
    lp_setup
        .b_mock
        .check_moax_balance(&users[1], &rust_biguint!(initial_balance));
    lp_setup
        .b_mock
        .check_moax_balance(&users[2], &rust_biguint!(initial_balance));
    ...
    lp_setup
        .b_mock
        .check_moax_balance(&lp_setup.owner_address, &rust_biguint!(NFT_TICKET_COST));
