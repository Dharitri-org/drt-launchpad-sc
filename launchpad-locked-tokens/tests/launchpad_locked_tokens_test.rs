...
use dharitri_sc::{
    ...
    types::{
        MoaxOrDctTokenIdentifier, DctLocalRole, DctTokenPayment, ManagedAddress,
        MultiValueEncoded,
    },
};
use dharitri_sc_scenario::{
    managed_address, managed_biguint, managed_moax_token_id, managed_token_id,
...
impl SimpleLockMock {
    ...
    fn call_lock_tokens(&self) {
        ...
        let locked_token_nonce = self.send().dct_nft_create_compact_named(
        ...
        let output_payment = DctTokenPayment::new(
            managed_token_id!(LOCKED_TOKEN_ID),
            locked_token_nonce,
            payment.amount,
        );
