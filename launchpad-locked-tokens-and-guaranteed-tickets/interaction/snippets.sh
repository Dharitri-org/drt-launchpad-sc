...
ADDRESS="moa1qqqqqqqqqqqqqpgqv70gqyr2tr2vqfrxxre6wzytt7nfcuyupr9swdy8cq"
...
SIMPLE_LOCK_ADDRESS="moa1qqqqqqqqqqqqqpgqxsyrdj38fclflak2mwtqdknqzqhuxy4s0n4s4flqw5"



build() {
    moapy contract clean ../../launchpad
    moapy contract build ../../launchpad
}
...
deploy() {
    ...
    moapy --verbose contract deploy --bytecode="../output/launchpad-locked-tokens-and-guaranteed-tickets.wasm" --recall-nonce --pem=${OWNER_PEM_PATH} \
    --gas-limit=200000000 \
    ...
    moapy data store --key=address-sc --value=${ADDRESS}
...
#   $2 = Amount in hex
addTickets() {
    local USER_ADDRESS_HEX="0x$(moapy wallet bech32 --decode $1)"

    moapy --verbose contract call ${ADDRESS} --recall-nonce --pem=${OWNER_PEM_PATH} \
...
#   $2 = Amount in hex
addTicketsPEM() {
    local USER_ADDRESS_HEX="0x$(moapy wallet pem-address-hex $1)"
...
depositLaunchpadTokens() {
    ...
    moapy --verbose contract call ${ADDRESS} --recall-nonce --pem=${OWNER_PEM_PATH} \
    --gas-limit=15000000 --function="DCTTransfer" \
...
confirmTicketsUser() {
    ...
    moapy --verbose contract call ${ADDRESS} --recall-nonce --pem=$1 --pem-index=$2\
...
claimLaunchpadTokensUser() {
    # no arguments needed
    moapy --verbose contract call ${ADDRESS} --recall-nonce --pem=$1\
...
upgrade() {
    ...
    moapy --verbose contract upgrade ${ADDRESS} \
...
#   $1 = Support address
setSupportAddress() {
    local NEW_SUPPORT_ADDRESS_HEX="0x$(moapy wallet bech32 --decode $1)"
...
#   $1 = New owner address
changeSCOwner() {
    local NEW_OWNER_ADDRESS_HEX="0x$(moapy wallet bech32 --decode $1)"
